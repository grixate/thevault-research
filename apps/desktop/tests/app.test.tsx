...
describe("App", () => {
  ...
  it("captures a quick note from the global shortcut", async () => {
    ...
    expect(screen.getByRole("button", { name: /save as source/i }).getAttribute("aria-pressed")).toBe("false");
  ...
  it("routes captured evidence from quick note into Storage intake", async () => {
    ...
    expect(screen.getByRole("button", { name: /save as source/i }).getAttribute("aria-pressed")).toBe("true");
  ...
  it("surfaces downloadable local model packs in settings", async () => {
    ...
    expect(within(commandCenter).getByText("Local models")).toBeTruthy();
    expect(within(commandCenter).getByText("0/3 essentials ready")).toBeTruthy();
    expect(within(commandCenter).getByText("Trusted models")).toBeTruthy();
    expect(within(commandCenter).getByText("0/1 packs, 0/1 runtimes")).toBeTruthy();
    expect(within(commandCenter).getByText("6 tasks need models")).toBeTruthy();
    expect(within(commandCenter).getByText("Starter models")).toBeTruthy();
    expect(within(commandCenter).getByText("Starter models can be prepared.")).toBeTruthy();
    expect(within(commandCenter).getByText("Items to finish")).toBeTruthy();
    expect(within(commandCenter).getByText("80 items")).toBeTruthy();
    expect(within(commandCenter).getByText("Connect local model tasks")).toBeTruthy();
    expect(within(commandCenter).getByText("Choose an approved local model for this task before using it.")).toBeTruthy();
    ...
    expect(within(commandCenter).getByRole("button", { name: /use starter setup/i })).toBeTruthy();
    expect(within(commandCenter).getByRole("button", { name: /^setup$/i })).toBeTruthy();
    expect(within(commandCenter).queryByRole("button", { name: /choose candidate files/i })).toBeNull();
    expect(within(commandCenter).getAllByRole("button")).toHaveLength(3);
    ...
    expect((await screen.findAllByText("Install the starter llama.cpp runtime to unlock local pipeline testing.")).length).toBeGreaterThan(1);
    ...
    expect(await screen.findByText("Runtimes")).toBeTruthy();
