...
- Quieted the Settings Models default state further on 2026-06-27: placeholder hardware/runtime strips, setup badges, helper sentences, and zero-count disclosure badges no longer render before real setup data exists; the first glance is now one setup action row plus quiet `Approval details` and `Model library` disclosures. Visual evidence: `/tmp/vault-settings-models-minimal-setup.png`.
...
- Quieted empty Settings Voice history on 2026-06-27: `Audio notes` and `Read-aloud history` sections now stay hidden until real audio assets exist. Visual evidence: `/tmp/vault-settings-voice-empty-history-hidden.png`.
...
- Quieted the Learning empty state on 2026-06-27: clean practice workspaces now keep one deck-creation panel and hide the inactive `Current card` inspector until cards exist. Visual evidence: `/tmp/vault-learning-empty-focused-deck.png`.
...
- Quieted Local tools review chrome on 2026-06-27: review affordances now appear only when a helper run actually created review items, and the action reads `Review` instead of `Review output`. Visual evidence: `/tmp/vault-local-tools-review-minimal.png`.
...
- Settings Voice hides empty voice-history sections until imported or generated audio exists.
...
- Generated-note status labels now use `Draft` and `Needs review` instead of exposing internal status strings.
...
- Empty Learning workspaces hide the inactive current-card inspector and keep focus on deck creation.
